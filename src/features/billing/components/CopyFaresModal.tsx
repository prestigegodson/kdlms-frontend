...
import { type TransportFareBranchCopyOutcome, copyTransportFares } from "@/api/billing";
...
/**
 * `CopyPricesModal`'s shape exactly, for the school-bus fare grid instead - lets an admin pick a
 * source session and one or more branches, then copies each selected branch's fares from that
 * session into the currently-viewed (target) session in one action
 * (`ManageTransportFaresUseCase.copyFromSession`).
 */
export function CopyFaresModal({ open, onClose, targetSessionId, onCopied }: CopyFaresModalProps) {
  ...
  const [outcomes, setOutcomes] = useState<TransportFareBranchCopyOutcome[] | null>(null);
  ...
  async function handleCopy() {
    ...
    setError(null);
    try {
      const result = await copyTransportFares(sourceSessionId, targetSessionId, Array.from(selectedBranchIds));
      setOutcomes(result.outcomes);
      if (result.outcomes.some((outcome) => outcome.success)) {
        onCopied();
      }
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Failed to copy fares");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <Modal open={open} onClose={handleClose} title="Copy from another session">
      <div className="space-y-4">
        ...
        {outcomes ? (
                ...
                <span>
                  ...
                  {outcome.success ? (
                    <span className="text-slate-600">
                      {" "}
                      &mdash; {outcome.copied} fare{outcome.copied === 1 ? "" : "s"} copied
          ...
          <>
            <FormField label="Source session" htmlFor="copy-fares-session">
              <Select
                id="copy-fares-session"
            ...
            <FormField label="Branches to copy" htmlFor="copy-fares-branches">
              <div
                id="copy-fares-branches"
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button type="button" variant="secondary" loading={submitting} disabled={!canCopy} onClick={handleCopy}>
                Copy fares
              </Button>
            </div>
          </>
