...
function renderAt(path: string) {
  const router = createMemoryRouter([{ path: "/take-home-quiz", element: <TakeHomeQuizPublicPage /> }], {
    initialEntries: [path],
  });
...
describe("TakeHomeQuizPublicPage", () => {
  ...
  it("shows Resume quiz instead of Start quiz when an attempt is already in progress", async () => {
    vi.mocked(publicApi.resolveTakeHomeQuiz).mockResolvedValue({ ...interstitial, attemptState: "IN_PROGRESS" });
    renderAt("/take-home-quiz?token=abc123");
  ...
  it("shows the submitted confirmation immediately when resolving an already-submitted attempt", async () => {
    vi.mocked(publicApi.resolveTakeHomeQuiz).mockResolvedValue({ ...interstitial, attemptState: "SUBMITTED" });
    renderAt("/take-home-quiz?token=abc123");
  ...
  it("shows a distinct message when the rate limiter rejects the request", async () => {
    vi.mocked(publicApi.resolveTakeHomeQuiz).mockRejectedValue(
      new ApiError(429, "Too many requests", { type: "https://kdlms.com/problems/too-many-requests" }),
    );
    renderAt("/take-home-quiz?token=abc123");
  ...
  it("counts down and auto-submits once the deadline passes", async () => {
    ...
    vi.mocked(publicApi.resolveTakeHomeQuiz).mockResolvedValue({ ...interstitial, timed: true, durationMinutes: 1 });
